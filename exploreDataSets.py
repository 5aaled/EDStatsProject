...
#  PROJET : ANALYSE DES DONNÉES MONDIALES SUR L'ÉDUCATION (EDStats)
# Objectif : Nettoyer, fusionner et explorer plusieurs fichiers de la base de données
#   afin d’analyser les indicateurs éducatifs par pays et par thématique.
...
EDStatsData=pd.read_csv('Source/EdStatsData.csv')                           # Données principales : indicateurs par pays et par année
EDStatsFootNote =  pd.read_csv('Source/EdStatsFootNote.csv')                # Notes explicatives associées aux données
EDStatsCountrySeries = pd.read_csv('Source/EdStatsCountry-Series.csv')      # Lien entre pays et indicateurs
EDStatsCountry = pd.read_csv('Source/EdStatsCountry.csv')                   # Métadonnées sur les pays (région, code, etc.)
EDStatsSeries= pd.read_csv('Source/EdStatsSeries.csv')                      # Dictionnaire des indicateurs (signification, thème, etc.)
# Explore the data
#  Exploration et nettoyage du fichier "EDStatsCountry"

# Aperçu général : dimensions, valeurs manquantes, infos descriptives
...
# Suppression des colonnes peu informatives ou redondantes
# (par exemple : colonnes vides, codes redondants, liens inutiles)
...
# Vérification des codes ISO des pays pour détecter les "faux pays"
valid_iso = {c.alpha_2 for c in pc.countries}     # Ensemble des codes ISO-2 valides
...
N_of_Real_countries  =  RealCountry_EDStatsCountry.shape[0]
print(N_of_Real_countries)
print(len(fake_country_Names))

# Remplissage manuel des valeurs manquantes dans la colonne "Region"
# (Ces pays existent réellement mais leur région est absente)
...
# 3️Nettoyage et filtrage du fichier "EDStatsSeries"
...
# Sélection des thématiques (topics) les plus importantes pour l'analyse
...
#  Fusion des jeux de données pour obtenir une table consolidée

# Fusion sur les clés communes : 'Country Code' et 'Indicator Code'
# On relie les informations des pays et les thèmes des indicateurs
...
# Suppression des colonnes inutiles après jointure
EDStatsData.drop(['Unnamed: 69',"Series Code"],inplace=True,axis=1) # here i deleted also Series Code that comes from the join

#  Nettoyage et préparation des autres fichiers secondaires
# Fusion pour ne conserver que les pays et indicateurs valides
...
#  Transformation des données et calculs de moyennes
# Récupération de toutes les colonnes correspondant à des années
year_cols = [c for c in EDStatsData.columns if c.isdigit()]

# Calcul de la moyenne des valeurs sur toutes les années pour chaque indicateur
EDStatsData['mean_value'] = EDStatsData[year_cols].mean(axis=1)


# Moyenne globale de tous les indicateurs par pays
EDStatsData_grouped_by_country = EDStatsData.groupby("Country Name")["mean_value"].mean()


# Moyenne des indicateurs regroupés par pays et par thématique
...
# Création de tables pivots pour visualiser les comparaisons
EDStatsData_pivot_data_by_indicator =EDStatsData.pivot(index="Country Name" ,columns ='Indicator Code',values='mean_value')


#  Analyse de corrélation entre thématiques et indicateurs
plt.figure(figsize=(8, 8))
sns.heatmap(EDStatsData_pivot_data_by_topic.corr(method="spearman"), cmap="coolwarm", center=0)
plt.title("Correlation Between Topic")
plt.show()
...
plt.figure(figsize=(10, 8))
sns.heatmap(EDStatsData_pivot_data_by_indicator.iloc[ : , 0 : 11].corr(method="spearman"), cmap="coolwarm", center=0)
...
# Résumé :
# - Nettoyage et fusion de 5 jeux de données du World Bank Education Stats.
# - Filtrage des pays réels et des thématiques pertinentes.
# - Calcul de moyennes par pays, par thématique et par indicateur.
# - Visualisation des corrélations entre indicateurs pour détecter les redondances.
